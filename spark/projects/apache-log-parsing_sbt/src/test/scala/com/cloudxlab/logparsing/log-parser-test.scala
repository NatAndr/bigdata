...
class LogParserSpec extends FlatSpec {
  ...
  "extractIP" should "Extract IP Address" in {
    ...
    val ip = logParser.extractByPattern(line, logParser.PATTERN_IP)
  ...
  "containsIP" should "Check if IP exists in the log line" in {
    ...
    assert(logParser.containsPattern(line1, logParser.PATTERN_IP))
    ...
    assert(!logParser.containsPattern(line2, logParser.PATTERN_IP))
  }
