...
class SampleTest extends FunSuite with SharedSparkContext {
  test("Computing top10 IP") {
    ...
    val records = utils.getTopNByPattern(rdd, sc, 10, utils.PATTERN_IP, sortAscending = false)
  ...
  test("Computing top10 URL") {
    ...
    val records = utils.getTopNByPattern(rdd, sc, 10, utils.PATTERN_URL, sortAscending = false)
    assert(records.length === 1)
